...
#include <stdio.h>
#include "GPURuntime/Graphics/Backend/Vulkan/VideoParser/Decoder/VulkanVideoParserIf.h"
#include "VulkanH264Decoder.h"
#include "GPURuntime/Graphics/Backend/Vulkan/VideoParser/STD/nvVulkanVideoUtils.h"
#include "GPURuntime/Graphics/Backend/Vulkan/VideoParser/STD/nvVulkanh264ScalingList.h"
...
bool VulkanH264Decoder::BeginPicture(VkParserPictureData *pnvpd)
{
    ...
    else
    {
        ...
        const seq_parameter_set_s * const sps = m_sps.get();
        const pic_parameter_set_s * const pps = m_pps.get();
        ...
        if ((pps->flags.weighted_pred_flag) && (slh->weights_out_of_range > 0) && (slh->slice_type != (int)slice_type_e::I) && (m_lErrorThreshold < 30))
        {
            cur->not_existing = true;
        }
...
bool VulkanH264Decoder::BeginPicture_SVC(VkParserPictureData *pnvpd)
{
    ...
    if (!init_sequence_svc(m_layer_data[DQIdMax].sps.get()))
        return false;
    ...
    for (int did = 0; did <= dependencyIdMax; did++)
    {
        ...
        if (m_dd->used)
        {
            ...
            m_dd->MaxDpbFrames = derive_MaxDpbFrames(m_dd->sps.get());
    ...
    size_t end_offset = m_VkPictureData.bitstreamDataLen;
    uint32_t maxCount = 0;
    const uint32_t* pSliceOffsets = m_VkPictureData.bitstreamData->GetStreamMarkersPtr(0, maxCount);
    uint32_t TotalSliceCnt = 0;
    uint32_t nNumSlices = m_VkPictureData.numSlices;
    ...
    for(uint32_t layer = 0; layer < 128; layer++)
    {
        ...
        (pnvpd + PicLayer)->bitstreamData = m_VkPictureData.bitstreamData;
        ...
        const seq_parameter_set_s* sps = m_layer_data[layer].sps.get();
        const pic_parameter_set_s* pps = m_layer_data[layer].pps.get();
...
bool VulkanH264Decoder::IsPictureBoundary(int32_t rbsp_size)
{
    ...
    SP<seq_parameter_set_s>* spss = &m_spss[0];
    ...
    if ((slhold == NULL) || (m_VideoSession.Buffer()->GetStreamMarkersCount() == 0))
        return true;
...
int32_t VulkanH264Decoder::ParseNalUnit()
{
    ...
    case NAL_UNIT_CODED_SLICE_IDR:
        if (slice_header(&slh, nal_ref_idc, nal_unit_type))
        {
            if (picture_boundary)
            {
                const seq_parameter_set_s *sps = m_spss[m_ppss[slh.pic_parameter_set_id]->seq_parameter_set_id].get();
            ...
            if ((slh.slice_type != (int)slice_type_e::I) && (slh.slice_type != (int)slice_type_e::SI))
                m_intra_pic_flag = 0;
            retval = NALU_SLICE;
        }
        break;
    ...
    case NAL_UNIT_CODED_SLICE_IDR_SCALABLE:
        if ((m_bUseMVC || m_bUseSVC) && (slice_header(&slh, nal_ref_idc, nal_unit_type)))
        {
            if (picture_boundary)
            {
                ...
                if (m_bUseSVC) {
                    sps = m_spssvcs[m_ppss[slh.pic_parameter_set_id]->seq_parameter_set_id].get();
                } else {
                    sps = m_spss[m_ppss[slh.pic_parameter_set_id]->seq_parameter_set_id].get();
                }
                                                  ...
                                                  seq_parameter_set_s *spssvc)
{
    ...
    ////////////////////////////////??????????/////////////////
    SP<seq_parameter_set_s> sps(spssvc);
    if (spssvc == nullptr) {
        sps = seq_parameter_set_s::Create(0);
    }
    ...
    int MaxDpbSize = derive_MaxDpbFrames(sps.get());
    ...
    sps->flags.seq_scaling_matrix_present_flag = sps->UpdateStdScalingList(sps.get(), &sps->stdScalingLists);
    ...
    if (sps->UpdateStdVui(sps.get(), &sps->stdVui, &sps->stdHrdParameters)) {
    ...
    if (spssvc == nullptr)
    {
        if ((spsNalUnitTarget == SPS_NAL_UNIT_TARGET_SPS) && m_outOfBandPictureParameters) {

            sps->SetSequenceCount(m_pParserData->spssClientUpdateCount[sps_id]++);
            m_VideoSession.AddVideoSessionParameters(sps);
        }
        m_spss[sps_id] = sps;
    }
...
bool VulkanH264Decoder::seq_parameter_set_svc_extension_rbsp()
{
    ...
    SP<seq_parameter_set_s> spssvc = seq_parameter_set_s::Create(0);

    int32_t sps_id = seq_parameter_set_rbsp(SPS_NAL_UNIT_TARGET_SPS_SVC, spssvc.get());
    ...
    if (m_outOfBandPictureParameters) {
        ...
        SP<StdVideoPictureParametersSet> picParamObj(spssvc);
        /////////////////////m_pClient->UpdatePictureParameters(picParamObj, spssvc->client);
    }
...
bool VulkanH264Decoder::seq_parameter_set_mvc_extension_rbsp(int32_t sps_id)
{
    ...
    if (m_outOfBandPictureParameters) {
        ...
        m_VideoSession.AddVideoSessionParameters(m_spss[sps_id]);
    }
    return true;
}
...
bool VulkanH264Decoder::pic_parameter_set_rbsp()
{
    ...
    SP<pic_parameter_set_s> pps = pic_parameter_set_s::Create(0);
    ...
    pps->flags.pic_scaling_matrix_present_flag = pps->UpdateStdScalingList(pps.get(), &pps->stdScalingLists);
    ...
    if (m_outOfBandPictureParameters) {
        ...
        SP<StdVideoPictureParametersSet> picParamObj(pps);
...
{
    SP<seq_parameter_set_s> sps;
    SP<pic_parameter_set_s> pps;
    ...
    if ((m_lErrorThreshold < 60) && (slh->slice_type == (int)slice_type_e::B) && (sps->profile_idc == 66))
    {
    ...
    if ((!sps->max_num_ref_frames) && (slh->slice_type != (int)slice_type_e::I) && (slh->slice_type != (int)slice_type_e::SI))
        return false;
    ...
    if (quality_id == 0)
    {
        if (slh->slice_type == (int)slice_type_e::B) {
            slh->direct_spatial_mv_pred_flag = flag();
        }
        if (slh->slice_type == (int)slice_type_e::P || slh->slice_type == (int)slice_type_e::SP || slh->slice_type == (int)slice_type_e::B)
        {
        ...
        if ((pps->flags.weighted_pred_flag && (slh->slice_type == (int)slice_type_e::P || slh->slice_type == (int)slice_type_e::SP)) || (pps->weighted_bipred_idc == 1 && slh->slice_type == (int)slice_type_e::B))
        {
    ...
    if (pps->flags.entropy_coding_mode_flag && slh->slice_type != (int)slice_type_e::I && slh->slice_type != (int)slice_type_e::SI)
        ue(); // cabac_init_idc
    se(); // slice_qp_delta
    if (slh->slice_type == (int)slice_type_e::SP || slh->slice_type == (int)slice_type_e::SI)
    {
        if (slh->slice_type == (int)slice_type_e::SP)
...
void VulkanH264Decoder::update_layer_info(SP<seq_parameter_set_s> sps, SP<pic_parameter_set_s> pps, slice_header_s *slh)
{  
...
bool VulkanH264Decoder::dpb_sequence_start(slice_header_s *slh)
{
    ...
    const seq_parameter_set_s* sps = m_sps.get();
...
void VulkanH264Decoder::dpb_picture_start(SP<pic_parameter_set_s> pps, slice_header_s *slh)
{
    m_slh = *slh;
    m_slh_prev = *slh;
    m_pps = pps;
    ...
    picture_order_count(m_sps.get(), &m_slh);
...
void VulkanH264Decoder::sei_payload(int payloadType, int payloadSize)
{
    ...
    case 0: // buffering_period (D.1.1)
        {
            ...
            {
                const seq_parameter_set_s *sps = m_spss[sps_id].get();
    ...
    case 1: // pic_timing (D.1.2)
        ...
        {
            const seq_parameter_set_s *sps = m_spss[m_last_sps_id].get();
