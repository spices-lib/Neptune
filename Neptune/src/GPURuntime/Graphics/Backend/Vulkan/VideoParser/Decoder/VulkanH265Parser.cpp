...
namespace {
    ...
    int CeilLog2(int n) { return (n > 0) ? Log2U31(n-1) : 0; }
...
bool VulkanH265Decoder::BeginPicture(VkParserPictureData *pnvpd)
{
    ...
    const auto sps = m_active_sps[m_nuh_layer_id];
    assert(sps);
    const auto pps = m_active_pps[m_nuh_layer_id];
    assert(pps);
    const auto vps = m_active_vps;
        ...
        if ((nal_unit_type >= NUT_TRAIL_N && nal_unit_type <= NUT_RASL_R) || (nal_unit_type >= NUT_BLA_W_LP && nal_unit_type <= NUT_CRA_NUT))
        {
            ...
            if (slice_header(nal_unit_type, nuh_temporal_id_plus1))
            {
                if (!m_bPictureStarted) // 1st slice - can't rely on first_slice_segment_in_pic_flag if there are data drops
                {
                    ...
                    std::shared_ptr<hevc_pic_param_s>& pps = m_ppss[slh->pic_parameter_set_id];
                    std::shared_ptr<hevc_seq_param_s>& sps = m_spss[pps->pps_seq_parameter_set_id];
...
{
    auto sps = hevc_seq_param_s::Create(0);

    sps->sps_video_parameter_set_id = (uint8_t)u(4);
    const auto vps = m_vpss[sps->sps_video_parameter_set_id];
...
{
    auto pps = hevc_pic_param_s::Create(0);
    ...
    const auto sps = m_spss[pps->pps_seq_parameter_set_id];
    ...
    if (m_outOfBandPictureParameters) {
        ...
        m_VideoSession.AddVideoSessionParameters(pps);
    }
...
void VulkanH265Decoder::video_parameter_set_rbsp()
{
    ...
    auto vps = hevc_video_param_s::Create(0);
    ...
    if (m_outOfBandPictureParameters ) {
        ...
        m_VideoSession.AddVideoSessionParameters(vps);
    }
...
bool VulkanH265Decoder::slice_header(int nal_unit_type, int nuh_temporal_id_plus1)
{
    ...
    const auto pps = m_ppss[pic_parameter_set_id];
    if (pps == nullptr) {
        return false;
    }
    ...
    if (m_nuh_layer_id > 0 && !vps->privFlags.default_ref_layers_active_flag &&
        ...
        if (slh->inter_layer_pred_enabled_flag && vps->numDirectRefLayers[m_nuh_layer_id] > 1) {
            if (!vps->privFlags.max_one_active_ref_layer_flag) {
                ...
                getNumActiveRefLayerPics(vps.get(), slh);
...
static int32_t GetMaxDpbSize(std::shared_ptr<hevc_seq_param_s>& sps)
{
    int32_t MaxLumaPS = 0;
...
bool VulkanH265Decoder::dpb_sequence_start(std::shared_ptr<hevc_seq_param_s>& sps)
{
...
void VulkanH265Decoder::dpb_picture_start(std::shared_ptr<hevc_pic_param_s>& pps, hevc_slice_header_s *slh)
{
    ...
    if (!cur->pPicBuf)
    {
        m_Client.AllocPictureBuffer(&cur->pPicBuf);
    }
    m_dpb_cur = cur;
