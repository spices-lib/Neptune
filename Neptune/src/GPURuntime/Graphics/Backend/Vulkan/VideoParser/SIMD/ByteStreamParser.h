...
#include "GPURuntime/Graphics/Backend/Vulkan/VideoParser/Decoder/VulkanVideoDecoder.h"
#include <stdarg.h>
#include <algorithm>
    ...
    bool VulkanVideoDecoder::ParseByteStreamSimd(const VkParserBitstreamPacket* pck, size_t* pParsedBytes)
    {
        ...
        if (pck->bDiscontinuity)
        {
            if (!m_bNoStartCodes)
            {
                ...
                m_VideoSession.Buffer()->SetSliceStartCodeAtOffset(m_nalu.end_offset);
        ...
        if (m_bNoStartCodes)
        {
            ...
            if (curr_data_size > 0)
            {
                ...
                m_VideoSession.Buffer()->WriteToBuffer(pdatain, curr_data_size, m_nalu.start_offset);
                ...
                if (ParseNalUnit() == NALU_SLICE)
                {
                    ...
                    m_VideoSession.Buffer()->AddStreamMarker(0);
        ...
        while (curr_data_size > 0) {
            ...
            if (data_used > 0)
            {
                ...
                if (bytes > 0) {
                    m_VideoSession.Buffer()->WriteToBuffer(pdatain, bytes, m_nalu.end_offset);
                }
        ...
        if (pck->bEOP || pck->bEOS)
        {
            if (m_nalu.start_offset == 0)
                m_llNaluStartLocation = m_llParsedBytes - m_nalu.end_offset;
            // Remove the trailing 00.00.01 from the NAL unit
            if ((m_nalu.end_offset >= 3) &&
                m_VideoSession.Buffer()->HasSliceStartCodeAtOffset(m_nalu.end_offset - 3))
            {
