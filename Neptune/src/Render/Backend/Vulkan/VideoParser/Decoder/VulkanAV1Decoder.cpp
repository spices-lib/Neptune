...
bool VulkanAV1Decoder::end_of_picture(uint32_t frameSize)
{
    ...
    m_VkPictureData = {};
    m_VkPictureData.numSlices = m_PicData.tileInfo.TileCols * m_PicData.tileInfo.TileRows;  // set number of tiles as AV1 doesn't have slice concept

    m_VkPictureData.bitstreamDataLen = frameSize;
    m_VkPictureData.bitstreamData = m_VideoSession.Buffer().get();
    m_VkPictureData.bitstreamDataOffset = 0; // TODO: The extra storage in this library and necessarily the app is silly.

    m_PicData.needsSessionReset = m_bSPSChanged;
    m_bSPSChanged = false;

    m_VkPictureData.firstSliceIndex = 0;
    memcpy(&m_VkPictureData.CodecSpecific.av1, &m_PicData, sizeof(m_PicData));
    m_VkPictureData.intra_pic_flag = (pStd->frame_type == STD_VIDEO_AV1_FRAME_TYPE_KEY);
...
bool VulkanAV1Decoder::BeginPicture(VkParserPictureData* pnvpd)
{
    ...
    m_Client.AllocPictureBuffer(&m_pCurrPic);
...
{
    auto prevSps = m_sps;
    m_sps = av1_seq_param_s::Create(spsSequenceCounter++);
    ...
    SP<StdVideoPictureParametersSet> picParamObj(m_sps);
    m_PicData.pStdSps = picParamObj.get();

    m_VideoSession.AddVideoSessionParameters(m_sps);
...
bool VulkanAV1Decoder::ParseByteStream(const VkParserBitstreamPacket* pck, size_t* pParsedBytes)
{
    ...
    if (m_VideoSession.Buffer()->HostData() == nullptr) {
    ...
    while (pdataStart < pdataEnd) {
        ...
        if (datasize > 0) {
            ...
            memcpy(m_VideoSession.Buffer()->HostData(), pdataStart, frame_size);
