...
namespace Neptune::Vulkan {
    ...
    bool VulkanVP9Decoder::ParseByteStream(const VkParserBitstreamPacket* pck, size_t* pParsedBytes)
    {
        ...
        m_VideoSession.CreateBuffer(m_bitstreamDataLen);
        m_VideoSession.Buffer()->ResetStreamMarkers();

        if (m_VideoSession.Buffer() == nullptr) {
        ...
        while ((dataSize > 0) || m_pictureStarted) {
            if (!m_pictureStarted) {
                ...
                if (dataSize >= (m_frameSize - m_nalu.end_offset)) {
                    memcpy(m_VideoSession.Buffer()->HostData() + m_nalu.end_offset, pDataIn, (size_t)(m_frameSize - m_nalu.end_offset));
                    ...
                    memcpy(m_VideoSession.Buffer()->HostData() + m_nalu.end_offset, pDataIn, (size_t)(dataSize));
                ...
                const uint8_t* data_start = m_VideoSession.Buffer()->HostData();
    ...
    bool VulkanVP9Decoder::ParseFrameHeader(uint32_t framesize)
    {
        ...
        m_VkPictureData = {};
        m_VkPictureData.CodecSpecific.vp9 = m_PicData;
        m_VkPictureData.numSlices = m_PicData.numTiles;
        m_VkPictureData.bitstreamDataLen = (framesize + addOffset + m_bufferSizeAlignment - 1) & ~(m_bufferSizeAlignment - 1); // buffer is already aligned so, no issues.
        m_VkPictureData.bitstreamData = m_VideoSession.Buffer().get();
        m_VkPictureData.bitstreamDataOffset = (size_t)(m_nalu.start_offset & ~((int64_t)m_bufferOffsetAlignment - 1));
