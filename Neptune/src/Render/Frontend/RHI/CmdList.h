...
namespace Neptune::RHI {
	...
	*/
	...
	public:
		...
		* @param[in] renderPass RenderPass.
		*/
		void SetRenderPass(SP<class RenderPass> renderPass) const { RHICmdList::m_Impl->SetRenderPass(renderPass); }
		...
		* @param[in] pipeline Pipeline.
		*/
		void CmdBindPipeline(SP<class Pipeline> pipeline) const { RHICmdList::m_Impl->CmdBindPipeline(pipeline); }
