/**
* @file Instrumentor.h.
* @brief The Instrumentor Class Definitions.
* @author Spices.
*/
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				std::stringstream ss;
				ss << "Instrumentor::BeginSession{ " << name << " } when session { " << m_CurrentSession->Name << " } already open.";
			...
			else
			{
				...
				std::stringstream ss;
				ss << "Instrumentor could not open results file { " << filepath << " }";
...
#define NEPTUNE_PROFILE 1
#if NEPTUNE_PROFILE
// Resolve which function signature macro will be used. Note that this only
// is resolved when the (pre)compiler starts, so the syntax highlighting
// could mark the wrong one in your editor!
#if defined(__GNUC__) || (defined(__MWERKS__) && (__MWERKS__ >= 0x3000)) || (defined(__ICC) && (__ICC >= 600)) || defined(__ghs__)
#define NEPTUEN_FUNC_SIG __PRETTY_FUNCTION__
#elif defined(__DMC__) && (__DMC__ >= 0x810)
#define NEPTUEN_FUNC_SIG __PRETTY_FUNCTION__
#elif (defined(__FUNCSIG__) || (_MSC_VER))
#define NEPTUEN_FUNC_SIG __FUNCSIG__
#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
#define NEPTUEN_FUNC_SIG __FUNCTION__
#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
#define NEPTUEN_FUNC_SIG __FUNC__
